...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative min-h-[calc(100vh-5rem-4rem)] overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-950 via-zinc-900 to-blue-950 text-zinc-50 shadow-[0_24px_80px_rgba(0,0,0,0.8)]">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center justify-between rounded-xl px-3 py-2 transition ${
                    active
                      ? "bg-blue-500/20 text-blue-100 ring-1 ring-blue-500/50"
                      : "text-zinc-300 hover:bg-zinc-900/70 hover:text-zinc-50"
                  }`}
                >
                  <span>{item.label}</span>
                  {item.badge && (
                    <span className="rounded-full bg-blue-500/25 px-2 py-0.5 text-[11px] text-blue-200">
                      {item.badge}
                    </span>
                  )}
                </Link>
